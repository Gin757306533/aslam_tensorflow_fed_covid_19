...
FedModel = namedtuple('FedModel', 'X Y DROP_RATE train_op loss_op acc_op logits pred Y_')
...
class Clients:
    def __init__(self, input_shape, num_classes, learning_rate, clients_num, local_client_number=1):
        ...
        self.dataset = Dataset(tf.keras.datasets.cifar10.load_data,
                        split=clients_num,
                            local_split=local_client_number
                            )
    ...
    def run_test2(self, num):
        ...
        return self.sess.run([self.model.pred, self.model.Y_],
                             feed_dict=feed_dict)
    ...
    def train_epoch(self, cid,lcid=0, batch_size=32, dropout_rate=0.5):
        """
            ...
            cid: Client id
        """
        dataset = self.dataset.train[cid][lcid]
        ...
        with self.graph.as_default():
            for _ in range(math.ceil(dataset.size // batch_size)):
                ...
                # print(self.sess.run(self.model.logits, feed_dict=feed_dict))
                # print(self.sess.run(self.model.Y, feed_dict=feed_dict))
    ...
    def get_local_clients_num(self):
        return len(self.dataset.train[0]) - 1

    def choose_local_clients(self, ratio=1.0):
        client_num = self.get_local_clients_num()
        choose_num = math.floor(client_num * ratio)
        return np.random.permutation(range(1, client_num))[:choose_num]